...
export default async function DecisionDashboard({ params }: { params: Promise<{ id: string }> }) {
  ...
  const coreAssumptions = decision.assumptions.filter((a: any) => !a.title.toLowerCase().includes('blind'));
  const blindSpots = decision.assumptions.filter((a: any) => a.title.toLowerCase().includes('blind'));
